...
<script>
    $(function() {
        ...
        // Success Type   
            toastr['success']
            ('<hr><div class="text-center"><form action="{{ route("admin.user.revert",session("toast-revert")) }}" method="POST"> @csrf <label for="revert"> برگرداندن </label> <button id="revert" class="btn btn-sm btn-secondary text-white btn-icon" title="برگرداندن"><i class="bx bx-reset"></i></button></form></div>',
             ' ریکورد مورد نظر حذف گردید', {
            closeButton: true,
            progressBar: true,
            showMethod: 'fadeIn',
            hideMethod: 'fadeOut',
